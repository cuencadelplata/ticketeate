...
export const handler = async (event: any, context: any) => {
  ...
  console.log('[lambda.handler DEBUG] event.headers keys:', Object.keys(event.headers || {}).join(','));
  console.log('[lambda.handler DEBUG] Authorization header:', event.headers?.authorization || event.headers?.Authorization || 'NOT FOUND');
