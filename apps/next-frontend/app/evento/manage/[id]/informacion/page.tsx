  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-4 space-y-2">
        <div><span className="font-medium">Título:</span> {event.titulo}</div>
        {event.descripcion && (
          <div><span className="font-medium">Descripción:</span> {event.descripcion}</div>
        )}
        {event.ubicacion && (
          <div><span className="font-medium">Ubicación:</span> {event.ubicacion}</div>
        )}
      </div>
    </div>
  );
