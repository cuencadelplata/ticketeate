...
export function SimplifiedDeploysPanel() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'building':
        return <Clock className="h-4 w-4 animate-pulse text-accent" />;
      default:
        return null;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'success':
        return <Badge className="border-primary/30 bg-primary/20 text-primary">Success</Badge>;
      case 'failed':
        return (
          <Badge className="border-destructive/30 bg-destructive/20 text-destructive">Failed</Badge>
        );
      case 'building':
        return <Badge className="border-accent/30 bg-accent/20 text-accent">Building</Badge>;
      default:
            ...
            <div className="h-[200px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="deploys" fill="hsl(var(--primary))" radius={2} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">{deploy.commit}</p>
