...
stats.get('/events', async c => {
  try {
    ...
    const eventsWithMetrics = eventsStats.map(event => {
      const totalStock = event.categorias_entrada.reduce((sum, cat) => sum + cat.stock_total, 0);
      const availableStock = event.categorias_entrada.reduce((sum, cat) => sum + cat.stock_disponible, 0);
      const soldStock = totalStock - availableStock;
      const avgPrice = event.categorias_entrada.length > 0 
        ? event.categorias_entrada.reduce((sum, cat) => sum + Number(cat.precio), 0) / event.categorias_entrada.length
        : 0;
      ...
      return {
        ...
        occupancyRate: totalStock > 0 ? Math.round((soldStock / totalStock) * 100) : 0
      };
    });
  ...
  try {
    ...
    const [
      ...
      prisma.reservas.groupBy({
        ...
        const total = results.reduce((sum, r) => sum + r._count.id_reserva, 0);
      ...
      prisma.reservas.count({
        ...
        prisma.reservas.count().then(total => 
    ...
    return c.json({
      metrics: {
        avgReservationsPerEvent: Math.round(avgReservationsPerEvent * 100) / 100,
